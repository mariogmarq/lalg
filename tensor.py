...
class Tensor:
    def __init__(
        ...
        _c_array=None,
        _requires_grad=True,
        _op: Optional[Ops] = None,
        _from: Optional[Tuple["Tensor", Optional[Union["Tensor", float]]]] = None,
    ):
        # Array stuff
        self._shape = shape
        ...
        self._array = (
            lalg.create_array(c_shape, self._n_dims) if _c_array is None else _c_array
        )
        ...
        if _requires_grad:
            ...
            if self._op in unary_ops:
                ...
                assert (
                    isinstance(_from[1], float)
                    if _op in binary_scalar
                    else isinstance(_from[1], Tensor)
                )
    ...
    def add(self, other) -> "Tensor":
        if isinstance(other, Tensor):
            assert all(
                [self._shape[i] == other._shape[i] for i in range(self._n_dims)]
            ), "Shapes do not match"
        ...
        if isinstance(other, Tensor):
            if self._n_dims > 2 or other._n_dims > 2:
                raise NotImplementedError(
                    "Multiplication of two tensors is not implemented"
                )
